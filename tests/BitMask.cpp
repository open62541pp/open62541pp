...
enum class BitMaskEnum : Underlying {
    One = 1,
    Two = 2,
};
...
TEST_CASE("Bitwise operations with enum (enabled with IsBitMaskEnum trait)") {
    ...
    SUBCASE("AND") {
        CHECK(static_cast<Underlying>(BitMaskEnum::One & BitMaskEnum::One) == 1);
        CHECK(static_cast<Underlying>(BitMaskEnum::One & BitMaskEnum::Two) == 0);
    }

    SUBCASE("OR") {
        CHECK(static_cast<Underlying>(BitMaskEnum::One | BitMaskEnum::One) == 1);
        CHECK(static_cast<Underlying>(BitMaskEnum::One | BitMaskEnum::Two) == 3);
    }

    SUBCASE("XOR") {
        CHECK(static_cast<Underlying>(BitMaskEnum::One ^ BitMaskEnum::One) == 0);
        CHECK(static_cast<Underlying>(BitMaskEnum::One ^ BitMaskEnum::Two) == 3);
    }

    SUBCASE("NOT") {
        CHECK(
            static_cast<Underlying>(~BitMaskEnum::One) == std::numeric_limits<Underlying>::max() - 1
        );
    }
...
TEST_CASE("BitMask") {
    SUBCASE("Constructors") {
        CHECK_NOTHROW(BitMask<BitMaskEnum>());
        CHECK_NOTHROW(BitMask<BitMaskEnum>(BitMaskEnum::One));
        CHECK_NOTHROW(BitMask<BitMaskEnum>(2));
    }
    ...
    SUBCASE("Conversion to underlying") {
        // CHECK(static_cast<Underlying>(BitMask<BitMaskEnum>(2)) == 2);  // deprecated
    }
